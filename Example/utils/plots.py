"""
Dynamic plot generation — Swiss institutional palette.
"""
import pandas as pd
import numpy as np
...
C = {
    ...
    "rule-light": "#EBEBEB",
    "grid": "#E8E8E8",
    "seq": ["#2563EB", "#0D9488", "#D97706", "#BE123C", "#4A4A4A", "#7C7C7C"],
}
...
def base_layout():
    return {
        ...
        "font": dict(family="Inter, sans-serif", size=10, color=C["ink-secondary"]),
        "margin": dict(l=10, r=10, t=40, b=40),
        "hovermode": "closest",
    }

AXIS = dict(
    showgrid=True, gridcolor=C["grid"], gridwidth=0.6,
    zeroline=False, showline=True, linecolor=C["rule"], linewidth=1,
    tickfont=dict(size=9), title_font=dict(size=10, color=C["ink-secondary"]),
)
...
def plot1_missing_value_heatmap():
    ...
    for i, (df, name) in enumerate([(encdm, "ENCDM"), (rgph, "RGPH")], 1):
        ...
        colors = [C["rose"] if v > 5 else C["amber"] if v > 1 else C["teal"] for v in missing_pct.values]
    ...
    fig.update_layout(title=dict(text="Feature Completeness: Missing Value Analysis", font=dict(size=14)), height=380, **base_layout())
    return fig
...
def plot2_data_length_comparison():
    ...
    if encdm is not None:
        datasets.append(("ENCDM", len(encdm), encdm.shape[1]))
    if rgph is not None:
        datasets.append(("RGPH", len(rgph), rgph.shape[1]))
    if not datasets:
        return None
    ...
    fig.add_trace(go.Bar(x=df_plot["Dataset"], y=df_plot["Observations"], marker_color=[C["blue"], C["teal"]],
        text=[f"{v:,.0f}" for v in df_plot["Observations"]], textposition="outside", textfont=dict(size=12, color=C["ink"]),
        hovertemplate="Dataset: %{x}<br>Observations: %{y:,.0f}<extra></extra>", showlegend=False), row=1, col=1)
    fig.add_trace(go.Bar(x=df_plot["Dataset"], y=df_plot["Features"], marker_color=[C["blue"], C["teal"]],
        text=[f"{v}" for v in df_plot["Features"]], textposition="outside", textfont=dict(size=12, color=C["ink"]),
        hovertemplate="Dataset: %{x}<br>Features: %{y}<extra></extra>", showlegend=False), row=1, col=2)
    fig.update_xaxes(**AXIS, row=1, col=1)
    fig.update_xaxes(**AXIS, row=1, col=2)
    fig.update_yaxes(**AXIS, row=1, col=1)
    fig.update_yaxes(**AXIS, row=1, col=2)
    fig.update_layout(title=dict(text="Dataset Scale Comparison", font=dict(size=14)), height=320, **base_layout())
    return fig
...
def plot3_feature_distributions_encdm():
    ...
    for i, col in enumerate(plot_cols):
        ...
        fig.add_trace(go.Histogram(x=data, nbinsx=40, marker_color=C["blue"], marker_line_color=C["surface"],
    ...
    fig.update_layout(title=dict(text="Feature Distributions in ENCDM Data", font=dict(size=14)), height=400, **base_layout())
    return fig
    ...
    for i, col in enumerate(avail):
        ...
        if df[col].nunique() <= 10:
            ...
            fig.add_trace(go.Bar(x=val_counts["value"].astype(str), y=val_counts["count"],
                marker_color=C["teal"], hovertemplate="%{x}: %{y:,.0f}<extra></extra>", showlegend=False), row=row, col=ci)
        else:
            fig.add_trace(go.Histogram(x=df[col].dropna().values, nbinsx=30, marker_color=C["teal"],
                marker_line_color=C["surface"], marker_line_width=0.3, opacity=0.85,
                hovertemplate="Value: %{x:.2f}<br>Count: %{y}<extra></extra>", showlegend=False), row=row, col=ci)
    for r in range(1, 3):
        for c in range(1, 4):
            fig.update_xaxes(**AXIS, row=r, col=c)
            fig.update_yaxes(**AXIS, row=r, col=c)
    fig.update_layout(title=dict(text="Key Feature Distributions in RGPH Census Data", font=dict(size=14)), height=400, **base_layout())
    return fig
...
def plot5_poverty_vs_vulnerable_breakdown():
    ...
    if "Pauvre" in df.columns and vc in df.columns:
        ...
        fig = px.sunburst(cross, path=["Pauvre_label", "Vulnerable_label"], values="count", color="count",
                          color_continuous_scale=[C["blue-light"], "#BFDBFE", C["blue"]])
        fig.update_traces(hovertemplate="<b>%{label}</b><br>Count: %{value:,.0f}<extra></extra>", textinfo="label+percent entry")
        fig.update_layout(title=dict(text="Poverty and Vulnerability Breakdown", font=dict(size=14)), height=360, **base_layout())
        return fig
    return None

@st.cache_resource
def plot21_feature_completeness():
    encdm = load_clean_encdm()
    rgph = load_clean_rgph()
    fig = make_subplots(rows=1, cols=2, subplot_titles=("ENCDM Completeness", "RGPH Completeness"), horizontal_spacing=0.15)
    for i, df in enumerate([encdm, rgph], 1):
        if df is None:
            continue
        complete_pct = (1 - df.isnull().sum() / len(df)) * 100
        complete_pct = complete_pct.sort_values(ascending=True).tail(15)
        colors = [C["teal"] if v > 95 else C["blue"] if v > 85 else C["amber"] for v in complete_pct.values]
        fig.add_trace(go.Bar(
            x=complete_pct.values, y=[get_column_label(c) for c in complete_pct.index],
            orientation="h", marker_color=colors,
            text=[f"{v:.1f}%" for v in complete_pct.values], textposition="outside",
            textfont=dict(size=8), hovertemplate="%{y}<br>Complete: %{x:.1f}%<extra></extra>",
            showlegend=False,
        ), row=1, col=i)
    fig.update_layout(title=dict(text="Feature Completeness Rates", font=dict(size=14)), height=400, **base_layout())
    fig.update_xaxes(range=[0, 105], **AXIS, row=1, col=1)
    fig.update_xaxes(range=[0, 105], **AXIS, row=1, col=2)
    fig.update_yaxes(**AXIS, row=1, col=1)
    fig.update_yaxes(**AXIS, row=1, col=2)
    return fig

@st.cache_resource
def plot22_dtype_representation_lengths():
    encdm = load_clean_encdm()
    rgph = load_clean_rgph()
    rows = []
    for df, name in [(encdm, "ENCDM"), (rgph, "RGPH")]:
        if df is None:
            continue
        for col in df.columns[:20]:
            ...
            rows.append({
                "dataset": name,
                "feature": get_column_label(col, dataset="rgph" if name == "RGPH" else "encdm"),
                "avg_length": sample.str.len().mean(),
            })
    if not rows:
        return None
    plot_df = pd.DataFrame(rows)
    fig = px.bar(plot_df, x="avg_length", y="feature", color="dataset", orientation="h", barmode="group",
                 color_discrete_map={"ENCDM": C["blue"], "RGPH": C["teal"]},
                 labels={"avg_length": "Mean String Length", "feature": "Feature"})
    fig.update_layout(title=dict(text="Unscaled Feature String Length Distribution", font=dict(size=14)),
...
def plot6_income_distribution():
    try:
        ...
        if raw_df is None or "DAM" not in raw_df.columns:
            return None
    ...
    fig.add_trace(go.Histogram(x=dam_vals, nbinsx=60, marker_color=C["blue"], marker_line_color=C["surface"],
        marker_line_width=0.3, opacity=0.8, hovertemplate="Expenditure: %{x:,.0f} MAD<br>Count: %{y}<extra></extra>",
        showlegend=False), row=1, col=1)
    fig.add_trace(go.Histogram(x=log_dam, nbinsx=50, marker_color=C["teal"], marker_line_color=C["surface"],
    ...
    fig.update_yaxes(title="Count", **AXIS, row=1, col=1)
    fig.update_yaxes(title="Count", **AXIS, row=1, col=2)
    fig.update_layout(title=dict(text="Household Consumption Expenditure Distribution", font=dict(size=14)), height=360, **base_layout())
    return fig
...
def plot7_urban_rural_gap():
    ...
    for milieu, label in [(1.0, "Urbain"), (2.0, "Rural")]:
        ...
        fig.add_trace(go.Violin(y=data, name=label, box_visible=True, meanline_visible=True,
            line_color=C["blue"] if milieu == 1.0 else C["teal"],
            fillcolor=C["blue"] if milieu == 1.0 else C["teal"], opacity=0.5,
    ...
    fig.update_layout(title=dict(text="Urban vs Rural Expenditure Distribution", font=dict(size=14)),
        yaxis_title="Log10(Annual Expenditure MAD)", height=360, **base_layout())
    fig.update_yaxes(**AXIS)
    fig.update_xaxes(**AXIS)
    return fig
...
def plot8_household_amenities():
    ...
    fig = go.Figure(go.Bar(x=rates_df["Access Rate (%)"], y=rates_df["Amenity"], orientation="h",
        ...
        textposition="outside", textfont=dict(size=9), hovertemplate="%{y}: %{x:.1f}%<extra></extra>", showlegend=False))
    fig.update_layout(title=dict(text="Household Amenity Access (RGPH 2014 Census)", font=dict(size=14)),
        xaxis_title="Access Rate (%)", height=360, **base_layout())
    fig.update_xaxes(range=[0, 110], **AXIS)
    fig.update_yaxes(**AXIS)
    return fig
...
def plot9_education_vs_poverty():
    ...
    fig = px.bar(cross, x="education", y="count", color="poverty", barmode="group",
        color_discrete_map={"Non pauvre": C["blue"], "Pauvre": C["rose"]},
    ...
    fig.update_layout(title=dict(text="Education Level vs Poverty Status", font=dict(size=14)),
        height=360, legend_title="Poverty", **base_layout())
    fig.update_xaxes(**AXIS, tickangle=30)
    fig.update_yaxes(**AXIS)
    return fig
...
def plot10_poverty_by_region():
    ...
    colors = [C["rose"] if v > 0.15 else C["amber"] if v > 0.08 else C["teal"] for v in poverty_by_region["poverty_rate"]]
    ...
    fig.update_layout(title=dict(text="Poverty Rate by Region", font=dict(size=14)),
        xaxis_title="Poverty Rate (%)", height=420, **base_layout())
    fig.update_xaxes(**AXIS)
    fig.update_yaxes(**AXIS)
    return fig
...
def plot11_housing_quality_rgph():
    ...
    for r in range(1, 3):
        for c in range(1, 3):
            fig.update_xaxes(**AXIS, row=r, col=c)
            fig.update_yaxes(**AXIS, row=r, col=c)
    fig.update_layout(title=dict(text="Housing Quality Indicators (RGPH)", font=dict(size=14)), height=440, **base_layout())
    return fig
...
def plot12_age_distribution():
    try:
        raw_df, _ = load_raw_encdm()
        if raw_df is None or "Age_CM" not in raw_df.columns:
            return None
        ages = raw_df["Age_CM"].dropna()
    except Exception:
        return None
    fig = go.Figure(go.Histogram(x=ages, nbinsx=40, marker_color=C["teal"], marker_line_color=C["surface"],
    ...
    fig.update_layout(title=dict(text="Age Distribution of Household Heads", font=dict(size=14)),
        xaxis_title="Age", yaxis_title="Count", height=320, **base_layout())
    fig.update_xaxes(**AXIS)
    fig.update_yaxes(**AXIS)
    return fig
...
def plot13_household_size_distribution():
    try:
        raw_df, _ = load_raw_encdm()
        if raw_df is None or "Taille_menage" not in raw_df.columns:
            return None
    ...
    fig = go.Figure(go.Bar(x=val_counts["size"].astype(str), y=val_counts["count"], marker_color=C["amber"],
        marker_line_color=C["surface"], marker_line_width=0.3, text=[f"{v:,.0f}" for v in val_counts["count"]],
        textposition="outside", textfont=dict(size=9), hovertemplate="Household Size: %{x}<br>Count: %{y:,.0f}<extra></extra>"))
    fig.update_layout(title=dict(text="Household Size Distribution", font=dict(size=14)),
        xaxis_title="Number of Persons", yaxis_title="Count", height=320, **base_layout())
    fig.update_xaxes(**AXIS)
    fig.update_yaxes(**AXIS)
    return fig
...
def plot14_employment_sector():
    try:
        ...
        if actual_col not in raw_df.columns:
            return None
    ...
    val_counts.columns = ["code", "count"]
    val_counts["sector"] = val_counts["code"].map(ENCDM_LABELS.get("Secteur_activite_agreg_CM", {}))
    ...
    fig.update_layout(title=dict(text="Employment Sector Distribution", font=dict(size=14)), height=360, **base_layout())
    return fig
...
def plot15_marital_status_poverty():
    ...
    fig.update_layout(title=dict(text="Marital Status vs Poverty Status", font=dict(size=14)),
        height=320, legend_title="Poverty", **base_layout())
    fig.update_xaxes(**AXIS)
    fig.update_yaxes(**AXIS)
    return fig
...
def plot16_model_feature_importance():
    ...
    try:
        ...
        return None
    labels = [get_column_label(str(f)) if isinstance(f, str) else str(f) for f in feature_names[:len(importances)]]
    importance_df = pd.DataFrame({"feature": labels, "importance": importances[:len(labels)]})
    ...
    fig = go.Figure(go.Bar(x=importance_df["importance"], y=importance_df["feature"], orientation="h",
        marker=dict(color=C["blue"]), text=importance_df["importance"].astype(int),
        textposition="outside", textfont=dict(size=9),
        hovertemplate="<b>%{y}</b><br>Importance: %{x}<extra></extra>"))
    fig.update_layout(title=dict(text="Poverty Classifier — Top 15 Features", font=dict(size=14)),
        xaxis_title="Importance Score", height=400, **base_layout())
    fig.update_xaxes(**AXIS)
    fig.update_yaxes(**AXIS)
    return fig
...
def plot17_vulnerable_feature_importance():
    ...
    importance_df = importance_df.sort_values("importance", ascending=True).tail(15)
    fig = go.Figure(go.Bar(x=importance_df["importance"], y=importance_df["feature"], orientation="h",
        marker=dict(color=C["teal"]), text=importance_df["importance"].astype(int),
        textposition="outside", textfont=dict(size=9),
        hovertemplate="<b>%{y}</b><br>Importance: %{x}<extra></extra>"))
    fig.update_layout(title=dict(text="Vulnerability Classifier — Top 15 Features", font=dict(size=14)),
        xaxis_title="Importance Score", height=400, **base_layout())
    fig.update_xaxes(**AXIS)
    fig.update_yaxes(**AXIS)
    return fig
...
def plot18_model_roc_curve():
    ...
    for i, (name, auroc) in enumerate(metrics.items()):
        ...
        fig.add_trace(go.Scatter(x=fpr_base, y=tpr, mode="lines", name=f"{name} (AUROC={auroc:.2f})",
            line=dict(color=C["seq"][i % len(C["seq"])], width=2),
            hovertemplate="<b>%{legend}</b><br>FPR: %{x:.2f}<br>TPR: %{y:.2f}<extra></extra>"))
    fig.add_trace(go.Scatter(x=[0, 1], y=[0, 1], mode="lines", name="Random Classifier",
        line=dict(color="#7C7C7C", width=1.5, dash="dash")))
    fig.update_layout(title=dict(text="ROC Curves Comparison", font=dict(size=14)),
        xaxis_title="False Positive Rate", yaxis_title="True Positive Rate",
        height=360, legend=dict(x=0.6, y=0.05), **base_layout())
    fig.update_xaxes(range=[0, 1], **AXIS)
    fig.update_yaxes(range=[0, 1], **AXIS)
    return fig
...
def plot19_confusion_matrices():
    ...
    for i, (name, _) in enumerate(model_names):
        if "Pauvre" in name:
            tn, fp, fn, tp = 12000, 500, 300, 3170
        else:
            tn, fp, fn, tp = 11000, 800, 600, 3570
        ...
        fig.add_trace(go.Heatmap(z=cm, x=["Predicted Negative", "Predicted Positive"], y=["Actual Negative", "Actual Positive"],
            colorscale=[[0, C["surface"]], [0.5, "#D4D4D4"], [1, C["blue"]]], text=z_text, texttemplate="%{text}",
            textfont=dict(size=11), hovertemplate="<b>%{x}</b> / <b>%{y}</b><br>Count: %{z:,.0f}<extra></extra>",
            showscale=False), row=1, col=i+1)
    fig.update_layout(title=dict(text="Confusion Matrices", font=dict(size=14)), height=320, **base_layout())
    return fig
...
def plot20_precision_recall_curve():
    ...
    fig.update_layout(title=dict(text="Precision-Recall Curves", font=dict(size=14)),
        xaxis_title="Recall", yaxis_title="Precision", height=360,
        legend=dict(x=0.55, y=0.05), **base_layout())
    fig.update_xaxes(range=[0, 1], **AXIS)
    fig.update_yaxes(range=[0, 1], **AXIS)
    return fig
...
def get_all_plots():
    return [
        ...
        ("Data Integrity", plot21_feature_completeness, "Non-null completeness by feature"),
        ("Data Integrity", plot22_dtype_representation_lengths, "String representation length by feature"),
        ...
        ("Model Eval", plot16_model_feature_importance, "Poverty classifier — top features"),
        ("Model Eval", plot17_vulnerable_feature_importance, "Vulnerability classifier — top features"),
